...
> **STATUS (2026-08-11): IMPLEMENTAÇÃO COMPLETA (Tasks 1-8).** Todas as migrations (17-20) aplicadas em produção, código das Tasks 3-5 deployado no Vercel, `detect_changes` revisado (risco medium, só nos símbolos esperados).
>
> **Achado durante a Task 6:** a auditoria original (seção 6 da spec, rodada antes de qualquer implementação) tinha um bug de paginação — só lia as primeiras 1000 linhas de `employees` via PostgREST sem `.range()`, e reportou "zero duplicatas" quando a base real tinha 3241 registros. A primeira tentativa de aplicar a migration 19 falhou com `duplicate key`. Reauditoria com paginação correta encontrou 2 matrículas de fato duplicadas (mesma pessoa lançada por engano tanto na unidade de origem quanto no GTE, no mesmo ciclo — erro de importação, confirmado com o usuário). Os 2 registros errados foram removidos manualmente (cascata apagou seus shifts/folgas; nenhum tinha `purchase_requests` vinculada) antes de reaplicar a migration 19 com sucesso. **Lição:** qualquer auditoria futura em tabelas grandes deve paginar explicitamente (`.range()`) — o default do PostgREST corta em 1000 linhas silenciosamente.
>
> **Task 8 (verificação de ponta a ponta):** como só existe 1 ciclo real no sistema (Julho/2026), não havia como testar uma transferência entre ciclos existentes. Simulação feita replicando exatamente a lógica de `handleConfirmImport` via service role key: 2 ciclos de teste (`RASCUNHO`, fora do seletor de "ciclo ativo"), 1 servidor de teste, shift no ciclo 1 (estabelecimento A, 15 plantões) e depois no ciclo 2 (estabelecimento B, 10 plantões, mesma matrícula). Resultado: 1 único `employee_id` preservado, `saldo_plantoes` correto (25-21=4), cada shift com `establishment_id` fixo no ciclo em que aconteceu, folga gerada pelo trigger com `establishment_id` = B (unidade do shift mais recente, confirma o fix da Task 2), e a query da "Folha por Servidor" retornando o servidor só no ciclo/estabelecimento correto em cada combinação testada. Todos os dados de teste (employee, shifts, folga, os 2 ciclos) foram apagados ao final — zero rastro em produção.
>
> **Pendente (não bloqueia, mas não foi feito por mim):** smoke test manual da Task 7 (logar como usuário ESTABELECIMENTO de verdade e confirmar que Relatórios/Folgas continuam corretos) — a service role key usada nesta implementação bypassa RLS, então não consegue validar isso.
