...
- **Distinção confirmada com o usuário durante o teste:** carga horária **em progresso** (saldo acumulando rumo a uma próxima folga) fica escondida pra escala só-Plus — isso não existe de verdade pra esse servidor. Folga **já gerada** é diferente: é um direito adquirido concreto ("se ele já tem folga gerada antes então é permitido"), então continua aparecendo normalmente em Lançamento de Plantões (badge no card, KPIs do topo, card "Folgas" do resumo do modal) e continua comprável/usufruível em Solicitar Compra — em nenhum dos dois lugares ela é escondida. Só o que está em progresso/acumulando (saldo, aba Plantões) é que some pra escala só-Plus.
