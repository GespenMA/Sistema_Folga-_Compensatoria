...
- **Ajustado durante o teste do usuário:** folga já `GERADA` antes da escala virar só-Plus continua válida e comprável/usufruível em Solicitar Compra normalmente (grandfather confirmado pelo usuário — "se ele já tem folga gerada antes então é permitido"). Mas nesta tela (Lançamento de Plantões) ela não deve aparecer de forma alguma — nem o badge "🎉 Folga disponível" no card, nem nos KPIs do topo ("Com Folga Pronta"/"Total de Folgas"), nem no card "Folgas" do resumo do modal. `temFolga` e as contagens agregadas passam a considerar `permite_carga_horaria` também, não só `folgasDisponiveis > 0`.
